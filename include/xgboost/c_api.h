...
/**
 * @brief Refresh an in-memory dense XGBDDPM DMatrix in place.
 *
 * This mutates the data and labels of a SimpleDMatrix and invalidates derived
 * page caches. It is an internal extension for XGBDDPM estimators.
 *
 * @param handle DMatrix handle.
 * @param x0 JSON encoded __array_interface__ for the original dense data.
 * @param y0 JSON encoded __array_interface__ for the original labels.
 * @param alpha_bars JSON encoded __array_interface__ for diffusion alpha bars.
 * @param config JSON config with noise_samples_per_row, timestep, target_index,
 *        objective, seed, and n_classes.
 *
 * @return 0 when success, -1 when failure happens.
 */
XGB_DLL int XGDMatrixXGBDDPMRefresh(DMatrixHandle handle, char const *x0, char const *y0,
                                    char const *alpha_bars, char const *config);

/**
 * @brief Refresh an in-memory dense XGBDiffusion DMatrix in place.
 *
 * This mutates the data and labels of a SimpleDMatrix and invalidates derived
 * page caches. It is an internal extension for XGBDiffusion estimators.
 *
 * @param handle DMatrix handle.
 * @param x0 JSON encoded __array_interface__ for the original dense data.
 * @param y0 JSON encoded __array_interface__ for the original labels.
 * @param alpha_bars JSON encoded __array_interface__ for VP alpha bars.
 * @param times JSON encoded __array_interface__ for training/sampling times.
 * @param config JSON config with noise_samples_per_row, timestep, target_index,
 *        diffusion_type, and seed.
 *
 * @return 0 when success, -1 when failure happens.
 */
...
/**
 * @brief Clear prediction caches held by the Booster.
 *
