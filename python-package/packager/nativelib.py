...
def build_libxgboost(
    ...
    """
    logger = logging.getLogger("xgb_diffusion.packager.build_libxgboost")
    ...
    def _build(*, generator: str, extra_cmake_args: Sequence[str] = ()) -> None:
        cmake_cmd = [
            "cmake",
            str(cpp_src_dir),
            generator,
    ...
    if system() == "Windows":
        ...
        supported_generators: List[Tuple[str, Sequence[str]]] = []
        if shutil.which("ninja"):
            supported_generators.append(("-GNinja", release_args))
        supported_generators.extend(
            [
                ("-GVisual Studio 18 2026", ()),
                ("-GVisual Studio 17 2022", ()),
                ("-GVisual Studio 16 2019", ()),
                ("-GVisual Studio 15 2017", ()),
                ("-GMinGW Makefiles", release_args),
            ]
        )
        for generator, extra_args in supported_generators:
            try:
                _build(generator=generator, extra_cmake_args=extra_args)
                logger.info(
...
def locate_or_build_libxgboost(
    ...
    logger = logging.getLogger("xgb_diffusion.packager.locate_or_build_libxgboost")
