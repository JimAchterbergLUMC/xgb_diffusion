"""
Custom build backend for the xgb-diffusion Python package.
...
def build_wheel(
    ...
    """Build a wheel"""
    logger = logging.getLogger("xgb_diffusion.packager.build_wheel")
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        pkg_path = workspace / PACKAGE_NAME
        copytree_with_logging(TOPLEVEL_DIR / PACKAGE_NAME, pkg_path, logger=logger)
...
def build_sdist(
    ...
    logger = logging.getLogger("xgb_diffusion.packager.build_sdist")

    if config_settings:
        raise NotImplementedError(
            "xgb-diffusion's custom build backend doesn't support config_settings option "
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        copytree_with_logging(
            TOPLEVEL_DIR / PACKAGE_NAME, workspace / PACKAGE_NAME, logger=logger
        )
        copytree_with_logging(
...
def build_editable(
    ...
    logger = logging.getLogger("xgb_diffusion.packager.build_editable")
    ...
    if locate_local_libxgboost(TOPLEVEL_DIR, logger=logger) is None:
        raise RuntimeError(
            ...
            "The built library must be available under the repository lib/ directory."
        )
