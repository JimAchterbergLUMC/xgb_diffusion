...
namespace {
double XGBDDPMUniform(std::uint64_t x) {
...
float XGBDDPMNormal(std::uint64_t seed, std::uint64_t row, std::uint64_t col) {
  auto u1 = XGBDDPMUniform(seed ^ (row * 0x9e3779b97f4a7c15ULL) ^ (col * 0xbf58476d1ce4e5b9ULL));
  auto u2 = XGBDDPMUniform(seed ^ (row * 0x94d049bb133111ebULL) ^ (col * 0x2545f4914f6cdd1dULL));
  double constexpr kTwoPi = 6.283185307179586476925286766559;
...
void SimpleDMatrix::XGBDDPMRefresh(ArrayInterface<2> const& x0, ArrayInterface<1> const& y0,
                                   ...
                                   bst_feature_t target_index, std::int32_t objective,
                                   std::uint64_t seed,
                                   std::vector<std::int64_t> const& n_classes) {
  CHECK_GE(noise_samples_per_row, 1);
  auto n_rows = x0.Shape<0>();
  auto n_cols = x0.Shape<1>();
  auto n_steps = alpha_bars.Shape<0>();
  CHECK_EQ(y0.Shape<0>(), n_rows);
  CHECK_LT(target_index, n_cols);

  bool all_steps = timestep < 0;
  CHECK(all_steps || static_cast<std::size_t>(timestep) < n_steps);
  auto out_cols = n_cols + (all_steps ? 1 : 0);
  auto rows_per_step = n_rows * static_cast<std::size_t>(noise_samples_per_row);
  auto out_rows = rows_per_step * (all_steps ? n_steps : 1);
  ...
  data.resize(out_rows * out_cols);
  for (std::size_t r = 0; r <= out_rows; ++r) {
    offset[r] = r * out_cols;
  }

  this->Info().labels.Reshape(out_rows, 1);
...
void SimpleDMatrix::XGBDiffusionRefresh(ArrayInterface<2> const& x0,
                                        ArrayInterface<1> const& y0,
                                        ArrayInterface<1> const& alpha_bars,
                                        ArrayInterface<1> const& times,
                                        std::int32_t noise_samples_per_row,
                                        std::int32_t timestep,
                                        bst_feature_t target_index,
                                        std::int32_t diffusion_type,
                                        std::uint64_t seed) {
  CHECK_GE(noise_samples_per_row, 1);
  auto n_rows = x0.Shape<0>();
  auto n_cols = x0.Shape<1>();
  auto n_steps = times.Shape<0>();
  CHECK_EQ(y0.Shape<0>(), n_rows);
  CHECK_EQ(alpha_bars.Shape<0>(), n_steps);
  CHECK_LT(target_index, n_cols);
...
BatchSet<GHistIndexMatrix> SimpleDMatrix::GetGradientIndex(Context const* ctx,
  ...
  auto regen = detail::RegenGHist(batch_param_, param);
  if (!gradient_index_ || regen) {
    ...
    if (xgbddpm_enabled_ && xgbddpm_cuts_ && batch_param_.Initialized() && !regen &&
        !ctx->IsCUDA()) {
      gradient_index_.reset(new GHistIndexMatrix{ctx, *sparse_page_,
                                                 this->Info().feature_types.ConstHostSpan(),
                                                 *xgbddpm_cuts_, param.max_bin, this->IsDense(),
                                                 param.sparse_thresh});
    } else if (!ctx->IsCUDA()) {
