...
XGB_DLL int XGDMatrixXGBDDPMRefresh(DMatrixHandle handle, char const *x0, char const *y0,
                                    char const *alpha_bars, char const *c_json_config) {
  API_BEGIN();
  CHECK_HANDLE();
  xgboost_CHECK_C_ARG_PTR(x0);
  xgboost_CHECK_C_ARG_PTR(y0);
  xgboost_CHECK_C_ARG_PTR(alpha_bars);
  xgboost_CHECK_C_ARG_PTR(c_json_config);

  auto *dmat = static_cast<std::shared_ptr<DMatrix> *>(handle)->get();
  auto *simple = dynamic_cast<data::SimpleDMatrix *>(dmat);
  CHECK(simple) << "XGBDDPM refresh only supports SimpleDMatrix.";

  ArrayInterface<2> x0_array{StringView{x0}};
  ArrayInterface<1> y0_array{StringView{y0}};
  ArrayInterface<1> alpha_array{StringView{alpha_bars}};
  Json config = Json::Load(StringView{c_json_config});
  auto noise_samples_per_row =
      static_cast<std::int32_t>(RequiredArg<Integer>(config, "noise_samples_per_row", __func__));
  auto timestep = static_cast<std::int32_t>(RequiredArg<Integer>(config, "timestep", __func__));
  auto target_index = static_cast<bst_feature_t>(
      static_cast<std::int64_t>(RequiredArg<Integer>(config, "target_index", __func__)));
  auto objective = static_cast<std::int32_t>(RequiredArg<Integer>(config, "objective", __func__));
  auto seed =
      static_cast<std::uint64_t>(static_cast<std::int64_t>(RequiredArg<Integer>(config, "seed", __func__)));

  std::vector<std::int64_t> n_classes;
  auto const &obj = get<Object const>(config);
  auto it = obj.find("n_classes");
  if (it != obj.cend()) {
    auto const &arr = get<Array const>(it->second);
    n_classes.reserve(arr.size());
    for (auto const &v : arr) {
      n_classes.push_back(get<Integer const>(v));
    }
  }

  simple->XGBDDPMRefresh(x0_array, y0_array, alpha_array, noise_samples_per_row, timestep,
                         target_index, objective, seed, n_classes);
  API_END();
}

XGB_DLL int XGDMatrixXGBDiffusionRefresh(DMatrixHandle handle, char const *x0, char const *y0,
                                         char const *alpha_bars, char const *times,
  ...
  xgboost_CHECK_C_ARG_PTR(y0);
  xgboost_CHECK_C_ARG_PTR(alpha_bars);
  xgboost_CHECK_C_ARG_PTR(times);
  xgboost_CHECK_C_ARG_PTR(c_json_config);

  auto *dmat = static_cast<std::shared_ptr<DMatrix> *>(handle)->get();
  auto *simple = dynamic_cast<data::SimpleDMatrix *>(dmat);
  CHECK(simple) << "XGBDiffusion refresh only supports SimpleDMatrix.";

  ArrayInterface<2> x0_array{StringView{x0}};
  ArrayInterface<1> y0_array{StringView{y0}};
  ArrayInterface<1> alpha_array{StringView{alpha_bars}};
  ArrayInterface<1> times_array{StringView{times}};
  Json config = Json::Load(StringView{c_json_config});
  auto noise_samples_per_row =
      static_cast<std::int32_t>(RequiredArg<Integer>(config, "noise_samples_per_row", __func__));
  auto timestep = static_cast<std::int32_t>(RequiredArg<Integer>(config, "timestep", __func__));
  auto target_index = static_cast<bst_feature_t>(
      static_cast<std::int64_t>(RequiredArg<Integer>(config, "target_index", __func__)));
  auto diffusion_type =
      static_cast<std::int32_t>(RequiredArg<Integer>(config, "diffusion_type", __func__));
  auto seed =
      static_cast<std::uint64_t>(static_cast<std::int64_t>(RequiredArg<Integer>(config, "seed", __func__)));

  simple->XGBDiffusionRefresh(x0_array, y0_array, alpha_array, times_array,
                              noise_samples_per_row, timestep, target_index, diffusion_type,
                              seed);
  API_END();
}
...
XGB_DLL int XGBoosterClearCaches(BoosterHandle handle) {
  API_BEGIN();
  CHECK_HANDLE();
  static_cast<Learner *>(handle)->ClearCaches();
  API_END();
}
